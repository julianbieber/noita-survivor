...
pub fn build(b: *std.Build) void {
    ...
    if (target.result.os.tag == std.Target.Os.Tag.freebsd) {
        ...
        exe_unit_tests.linkSystemLibrary("glfw3");
    }
    ...
    if (target.result.os.tag == std.Target.Os.Tag.freebsd) {
        exe_check.linkSystemLibrary("glfw");
    } else if (target.result.os.tag == std.Target.Os.Tag.linux) {
        exe_check.linkSystemLibrary("glfw3");
    }
    exe_check.linkLibC(); // Any other code to define dependencies would
    // probably be here.
